...
class FirstPersonCamera {
  ...
  updateHeadBob_(timeElapsedS) {
    if (this.headBobActive_) {
      ...
      const nextStep = 1 + Math.floor(((this.headBobTimer_)) / wavelength);
      const nextStepTime = nextStep * wavelength /10 + 0.5;
...
class FirstPersonCameraDemo {
  ...
  initializeScene_() {
    ...
    checkerboard.repeat.set(2, 2);
    checkerboard.encoding = THREE.sRGBEncoding;

    const box = new THREE.Mesh(
      new THREE.BoxGeometry(8, 12, 0.1),
      this.loadMaterial_('concrete1-', 1));
    box.rotation.set(0, 6, 0);
    box.position.set(6, 0, -40);
    box.castShadow = true;
    ...
    const box1 = new THREE.Mesh(
      new THREE.BoxGeometry(8, 12, 0.1),
      this.loadMaterial_('concrete2-', 1));
      box1.rotation.set(0, 4, 0);
    box1.position.set(-4, 4, -50);
    box1.castShadow = true;
    box1.receiveShadow = true;
    this.scene_.add(box1);

    const box2 = new THREE.Mesh(
      new THREE.BoxGeometry(12, 8, 0.1),
      this.loadMaterial_('concrete3-', 1));
      box2.rotation.set(0, 3, 0);
    box2.position.set(5, 2, -60);
  ...
  const crosshair = mapLoader.load('resources/crosshair.png');
    ...
    const pointer1 = mapLoader.load('resources/hud/pointer1.png');
    pointer1.anisotropy = maxAnisotropy;

    this.sprite_ = new THREE.Sprite(
      new THREE.SpriteMaterial({map: pointer1, color: 0xffffff, fog: false, depthTest: false, depthWrite: false}));
    this.sprite_.scale.set(3.8, 2 * this.camera_.aspect, 1)
    this.sprite_.position.set(0, 0, -10);
  ...
  initializeLights_() {
    ...
    let light = new THREE.SpotLight(
        0xFFFFFF, 5.0, distance, angle, penumbra, decay);
    ...
    light.position.set(0, 65, -40);
    light.lookAt(5, 2, -60);
    this.scene_.add(light);

    const upColour = 0xFFFF80;
    const downColour = 0x808080;
    light = new THREE.AmbientLight(0xFFFFFF, 1.3);
    this.scene_.add(light);
  }
